...
static ngx_int_t ngx_rtmp_live_send_message(ngx_rtmp_session_t *s, ngx_chain_t *out, unsigned int priority){
	return ngx_rtmp_send_message(s, out, priority);
}
static ngx_chain_t*   ngx_rtmp_live_prepare_message(ngx_rtmp_session_t *s, ngx_rtmp_header_t *h, ngx_rtmp_header_t *lh,
		ngx_chain_t *in){
	...
	if(rpkt){
		ngx_rtmp_prepare_message(s, h, lh, rpkt);
	}
	return rpkt;
}
...
ngx_rtmp_live_join(ngx_rtmp_session_t *s, u_char *name, unsigned publisher)
{
    ...
    ctx->session = s;
	ctx->send_message_pt = ngx_rtmp_live_send_message;
	ctx->prepare_message_pt = ngx_rtmp_live_prepare_message;
	ctx->send_meta_data_pt = ngx_rtmp_live_send_message;
	ctx->live_type = NGX_LIVE_TYPE_RTMP;
...
*/
	...
	for (pctx = ctx->stream->ctx; pctx; pctx = pctx->next) {
		...
		if (meta && meta_version != pctx->meta_version) {
			ngx_log_debug0(NGX_LOG_DEBUG_RTMP, ss->connection->log, 0,
						 "live: meta");
			if (pctx->send_meta_data_pt(ss, meta, 0) == NGX_OK) {
				pctx->meta_version = meta_version;
			}			
		}
		rpkt = pctx->prepare_message_pt(s, &ch, &lh, in);
        ...
        if (!cs->active) {
                ...
                !pctx->cs[1].active)
            {
				...
				aapkt = pctx->prepare_message_pt(s, &clh, NULL, aapkt);
            }
            ...
            if (header || coheader) {
                ...
                if (header) {
                    if (apkt == NULL) {            
						apkt = pctx->prepare_message_pt(s, &lh, NULL, header);					
                    }

                    rc = pctx->send_message_pt(ss, apkt, 0);
                    if (rc != NGX_OK) {
                        continue;
                    }
                }
                ...
                if (coheader) {
                    if (acopkt == NULL) {         
						acopkt = pctx->prepare_message_pt(s, &clh, NULL, coheader);
                    }

                    rc = pctx->send_message_pt(ss, acopkt, 0);
                    if (rc != NGX_OK) {
                        continue;
                    }

                } else if (dummy_audio) {
                    pctx->send_message_pt(ss, aapkt, 0);
                }
                ...
                /* send absolute packet 当发送出现丢包延时*/
                ngx_log_debug2(NGX_LOG_DEBUG_RTMP, ss->connection->log, 0,
                               "live: abs %s packet timestamp=%uD",
                               type_s, ch.timestamp);

                if (apkt == NULL) {
					apkt = pctx->prepare_message_pt(s, &ch, NULL, in);
                }

                rc = pctx->send_message_pt(ss, apkt, prio);
                if (rc != NGX_OK) {
                    continue;
                }
		...
		if (pctx->send_message_pt(ss, rpkt, prio) != NGX_OK) {
		    ++pctx->ndropped;
...
#if 0
static ngx_int_t
ngx_rtmp_live_av2(ngx_rtmp_session_t *s, ngx_rtmp_header_t *h,
                 ngx_chain_t *in)
{
    ngx_rtmp_live_ctx_t            *ctx, *pctx;
    ngx_rtmp_codec_ctx_t           *codec_ctx;
